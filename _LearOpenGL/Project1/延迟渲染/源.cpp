...
#pragma region 帧率缓冲
	...
	glBindTexture(GL_TEXTURE_2D, gPosition);//位置信息存储的精度要高一些，所以是GL_RGB16F
	...
	//深度缓冲对象
	//渲染缓冲对象存放的数据都是原始数据 所以交换数据的时候会很快
	//通常对于深度测试和模板测试等只读的缓冲区使用渲染缓冲对象会更加优化
...
#pragma endregion
	...
	Shader lightShader("lightver.txt", "lightfrag.txt");
...
#pragma region 渲染循环
	...
	while (!glfwWindowShouldClose(window))
	{
	   ...
	   // --------------------
		...
		//渲染所有灯光(使用的默认缓冲)
		//把g-buffer里面的深度信息复制给默认缓冲中,用于后面的灯光渲染
		glBindFramebuffer(GL_READ_FRAMEBUFFER, gBuffer);
		glBindFramebuffer(GL_DRAW_FRAMEBUFFER, 0);
		glBlitFramebuffer(0, 0, SCR_WIDTH, SCR_HEIGHT, 0, 0, SCR_WIDTH, SCR_HEIGHT, GL_DEPTH_BUFFER_BIT, GL_NEAREST);
		glBindFramebuffer(GL_FRAMEBUFFER, 0);//绑定默认缓冲
